...
const to_buffer_1 = __importDefault(require("to-buffer"));
const crypto_1 = __importDefault(require("crypto"));
const length_prefixed_message_1 = __importDefault(require("length-prefixed-message"));
...
function Swarm(opts) {
    ...
    this.connections = [];
    this.id = opts.id || crypto_1.default.randomBytes(32);
    ...
    this._tcp =
        opts.tcp === false
            ? null
            : net_1.default.createServer().on("connection", onconnection);
    this._utp =
        opts.utp === false || !utp ? null : utp().on("connection", onconnection);
    this._tcpConnections = this._tcp && (0, connections_1.default)(this._tcp);
    this._adding = null;
    ...
    function onconnection(connection) {
        ...
        (0, debug_1.default)("inbound connection type=%s ip=%s:%d", type, connection.remoteAddress, connection.remotePort);
        connection.on("error", onerror);
        self.totalConnections++;
        self._onconnection(connection, type, null);
    }
}
util_1.default.inherits(Swarm, events_1.default.EventEmitter);
Swarm.prototype.close = Swarm.prototype.destroy = function (onclose) {
    if (this.destroyed)
        return process.nextTick(onclose || noop);
    if (onclose)
        this.once("close", onclose);
    if (this._listening && this._adding)
        return this.once("listening", this.destroy);
    this.destroyed = true;
    if (this._discovery)
    ...
    if (this._listening) {
        if (this._tcp)
            this._tcp.close(onserverclose);
        if (this._utp)
            this._utp.close(onserverclose);
    }
    else {
        this.emit("close");
    }
...
Swarm.prototype.join = function (name, opts, cb) {
    if (typeof opts === "function")
        return this.join(name, {}, opts);
    name = (0, to_buffer_1.default)(name);
    if (!opts)
        opts = {};
    if (typeof opts.announce === "undefined")
        opts.announce = true;
    if (!this._listening && !this._adding)
        this._listenNext();
    if (this._adding) {
...
Swarm.prototype.addPeer = function (name, peer) {
    peer = peerify(peer, (0, to_buffer_1.default)(name));
    if (this._peersSeen[peer.id])
        return;
    if (this._whitelist.length && this._whitelist.indexOf(peer.host) === -1)
        return;
...
Swarm.prototype._ondiscover = function () {
    ...
    if (this._options.dns !== false) {
        if (!this._options.dns || this._options.dns === true)
            this._options.dns = {};
        this._options.dns.socket = this._utp;
    }
    if (this._options.dht !== false) {
        if (!this._options.dht || this._options.dht === true)
            this._options.dht = {};
        this._options.dht.socket = this._utp;
    }
    this._discovery = (0, Discovery_1.default)(this._options);
    ...
    for (var i = 0; i < joins.length; i++)
        this.join(joins[i].name, joins[i].opts, joins[i].cb);
...
Swarm.prototype._kick = function () {
    if (this.maxConnections && this.totalConnections >= this.maxConnections)
        return;
    if (this.destroyed)
        return;
    var self = this;
    ...
    (0, debug_1.default)("connecting %s retries=%d", next.id, next.retries);
    var tcpSocket = null;
    var utpSocket = null;
    var tcpClosed = true;
    var utpClosed = true;
    if (this._tcp) {
        tcpClosed = false;
        tcpSocket = net_1.default.connect(next.port, next.host);
    ...
    function ondeferredconnect() {
        if (!self._tcp || tcpClosed)
            return onconnect.call(utpSocket);
        setTimeout(function () {
            if (!utpClosed && !connected)
                onconnect.call(utpSocket);
        }, 500);
    }
    function ontimeout() {
        (0, debug_1.default)("timeout %s", next.id);
        didTimeOut = true;
        if (utpSocket)
            utpSocket.destroy();
        if (tcpSocket)
    ...
    function cleanup() {
        ...
        if (utpSocket)
            utpSocket.removeListener("close", onclose);
        if (tcpSocket)
    ...
    function onclose() {
        ...
        if (tcpClosed && utpClosed) {
            (0, debug_1.default)("onclose utp+tcp %s will-requeue=%d", next.id, !connected);
            cleanup();
            if (!connected) {
                self.totalConnections--;
    ...
    function onconnect() {
        ...
        (0, debug_1.default)("onconnect %s type=%s", next.id, type);
        if (type === "utp" && tcpSocket)
            tcpSocket.destroy();
        if (type === "tcp" && utpSocket)
            utpSocket.destroy();
...
Swarm.prototype._onconnection = function (connection, type, peer) {
    ...
    if (this._stream) {
        ...
        if (connection.id)
            idHex = connection.id.toString("hex");
        connection.on("handshake", onhandshake);
        if (this._options.connect)
            this._options.connect(connection, wire);
        else
            (0, pump_1.default)(wire, connection, wire);
    }
    else {
    ...
    function onclose() {
        ...
        if (i > -1) {
            ...
            if (last !== connection)
                self.connections[i] = last;
        }
        if (remoteIdHex &&
            self._peersIds[remoteIdHex] &&
            self._peersIds[remoteIdHex].connection === connection) {
    ...
    function onhandshake(remoteId) {
        ...
        if (Buffer.isBuffer(connection.discoveryKey) ||
            Buffer.isBuffer(connection.channel)) {
        ...
        if (old) {
            (0, debug_1.default)("duplicate connections detected in handshake, dropping one");
            if (!(oldType === "utp" && type === "tcp")) {
                if ((peer && remoteIdHex < idHex) ||
                    (!peer && remoteIdHex > idHex) ||
                    (type === "utp" && oldType === "tcp")) {
                    self.emit("redundant-connection", connection, info);
                    connection.destroy();
                    return;
                }
...
Swarm.prototype.listen = function (port, onlistening) {
    if (this.destroyed)
        return;
    if (this._tcp && this._utp)
        return this._listenBoth(port, onlistening);
    if (!port)
        port = 0;
    if (onlistening)
...
function handshake(socket, id, cb) {
    length_prefixed_message_1.default.write(socket, id);
    length_prefixed_message_1.default.read(socket, cb);
}
function onerror() {
    this.destroy();
}
