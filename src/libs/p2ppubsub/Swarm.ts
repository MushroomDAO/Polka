...
function Swarm(opts?: any) {
	...
	this._tcp =
		opts.tcp === false
			? null
			: net.createServer().on("connection", onconnection);
	this._utp =
	...
	function onconnection(connection) {
		...
		debug(
			"inbound connection type=%s ip=%s:%d",
			type,
...
Swarm.prototype.join = function (name, opts, cb) {
	...
	if (this._adding) {
		...
		this._discovery.join(
			name,
			port,
			{ impliedPort: opts.announce && !!this._utp },
			cb
		);
	}
};
...
Swarm.prototype._requeue = function (peer) {
	...
	var self = this;
	var wait =
		peer.retries >= RECONNECT_WAIT.length ? 0 : RECONNECT_WAIT[peer.retries++];
...
Swarm.prototype._onconnection = function (connection, type, peer) {
	...
	function onhandshake(remoteId) {
		...
		if (
			Buffer.isBuffer(connection.discoveryKey) ||
			Buffer.isBuffer(connection.channel)
		) {
			var suffix =
				"@" + (connection.discoveryKey || connection.channel).toString("hex");
			remoteIdHex += suffix;
			idHex += suffix;
		}
