...
function Discovery(opts) {
	...
	function ondhtpeer(peer, infoHash, via) {
		...
		if (via)
			debug(
				"chan=%s dht discovery peer=%s:%s via=%s:%s",
				prettyHash(id),
				peer.host,
				peer.port,
				via.host || via.address,
				via.port
			);
		else
			debug(
				"chan=%s dht discovery peer=%s:%s",
				prettyHash(id),
				peer.host,
				peer.port
			);
...
Discovery.prototype.join = function (id, port, opts, cb) {
	...
	function dns() {
		if (announcing) {
			...
			self.dns.announce(
				hashHex,
				port,
				{ publicPort: publicPort, multicast: !skipMulticast },
				queryDone
			);
		} else {
		...
		dnsTimeout = setTimeout(
			dns,
			self._dnsInterval || (60 * 1000 + Math.random() * 10 * 1000) | 0
		);
	}
	...
	function dht() {
		...
		dhtTimeout = setTimeout(
			dht,
			self._dhtInterval || (10 * 60 * 1000 + Math.random() * 5 * 60 * 1000) | 0
		);
	}
};
